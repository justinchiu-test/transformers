...
# Use SharedExperts instead of GptOssExperts
GptOssExperts = SharedExperts


# Use SharedRouter instead of GptOssTopKRouter
GptOssTopKRouter = SharedRouter


@use_kernel_forward_from_hub("MegaBlocksMoeMLP")
class GptOssMLP(SharedMoE):
    """GPT-OSS MLP using SharedMoE."""
    def __init__(self, config):
        # Ensure GPT-OSS specific settings
        config.use_fused_experts = True
        config.router_bias = True
        config.moe_alpha = getattr(config, 'moe_alpha', 1.702)
        config.moe_limit = getattr(config, 'moe_limit', 7.0)
...
class GptOssDecoderLayer(SharedDecoderLayer):
    def __init__(self, config: GptOssConfig, layer_idx: int):
        # Set use_sinks flag before calling parent init
        config.use_sinks = True
        super().__init__(config, layer_idx)
        # Replace the self_attn with GptOssAttention and mlp with GptOssMLP
